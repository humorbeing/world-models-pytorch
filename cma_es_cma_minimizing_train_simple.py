...
def runs(para):
    ...
    for _ in range(5):
        ...
        rewas.append(r)
    # print(rewas)
    return (-1)*np.mean(rewas)

c_sv = '/media/ray/SSD/workspace/python/dataset/save_here/C/c.save.npy'
para = np.load(c_sv)
# print(para.shape)
es = cma.CMAEvolutionStrategy(para, 2, inopts={"popsize": 20})
prev_best_fitness = 999.9
for generation in range(100):
    params_set, fitnesses = es.ask_and_eval(runs)
    ...
    if best_overall_fitness < prev_best_fitness:
        ...
        # best_para = best_overall_params
        np.save(c_sv, best_overall_params)